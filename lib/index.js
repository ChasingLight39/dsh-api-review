...
import { AuditLogger as AuditLoggerImpl } from "./audit.js";
import { settleCheckpoint } from "./checkpoint.js";
import { assistantText, blockToText, configForCategory, makeUnit, resolveRootSessionId, } from "./collectors.js";
import { Config, validateConfig } from "./config.js";
import { ReviewEngine } from "./engine.js";
import { tryUnfreeze } from "./freeze.js";
import { GuardClient } from "./guard-client.js";
import { deleteRootState, getOrCreateRootState } from "./root-state.js";
...
async function processUnit(engine, unit, ctx, agent, signal) {
    if (unit.block) {
...
export function apply(ctx, rawConfig) {
    ...
    const audit = new AuditLoggerImpl(config.auditLogDir);
    const guard = new GuardClient(config);
    let reviewEngine;
    const scheduler = new ReviewScheduler(guard, (task, result) => reviewEngine.handleAsyncResult(task, result), config.maxQueueSize);
    ...
    ctx.on('agent/pre-step', async ({ agent, messages, signal }, next) => {
        ...
        for (const message of messages) {
            const category = message.source.kind === 'user' ? 'user' : 'unmatched';
            const cfg = configForCategory(config, category);
            if (!cfg.review && !cfg.block && !cfg.includeContext)
                continue;
