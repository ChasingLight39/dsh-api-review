...
async function withCheckpointLock(state, fn) {
    let acquired = false;
    let waitPromise;
...
function formatPendingBlocks(blocks) {
    return blocks.map((block, index) => {
        return `${index + 1}. [${block.category}] ${block.reason}\n   ${block.contentPreview}`;
    }).join('\n');
}
async function askCheckpointUser(ctx, agent, blocks, signal, fallback, audit) {
 ...
 */
export async function settleCheckpoint(ctx, agent, state, config, audit, signal) {
    return withCheckpointLock(state, async () => {
        ...
        if (stateSnapshot.frozen)
            return 'deny';
        if (!stateSnapshot.hasPending)
            return 'allow';
        ...
        const rootAgent = ctx.agents.get(state.rootSessionId) ?? agent;
