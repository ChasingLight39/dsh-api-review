...
import { settleCheckpoint } from "./checkpoint.js";
import { appendUnit, snapshotContext, trimContextIfNeeded, unitTokens } from "./context.js";
import { getOrCreateRootState, getRootState, withRootLock } from "./root-state.js";
...
export class ReviewEngine {
    ...
    audit;
    scheduler;
    constructor(config, audit, scheduler) {
        this.config = config;
        this.audit = audit;
        this.scheduler = scheduler;
    }
    async setSystemSnapshot(rootSessionId, agentId, content) {
    ...
    async systemSnapshotFor(rootSessionId, agentId) {
        if (!this.config.system.includeContext)
            return undefined;
        const state = getOrCreateRootState(rootSessionId);
        return state.systemSnapshots.get(agentId);
    }
    async getContextSnapshot(rootSessionId) {
     ...
     */
    async reviewAsyncExternal(unit, snapshot, systemSnapshot) {
        if (!unit.review)
            return;
        this.scheduler.enqueue({
            taskId: randomUUID(),
     ...
     * system units.
     */
    async reviewBlockingExternal(ctx, agent, unit, snapshot, systemSnapshot, signal) {
        const state = getOrCreateRootState(unit.rootSessionId);
        const prior = await settleCheckpoint(ctx, agent, state, this.config, this.audit, signal);
        if (prior === 'deny')
            return { allow: false, reason: 'frozen by prior pending blocks' };
        if (!unit.review)
            return { allow: true };
        const task = {
            taskId: randomUUID(),
        ...
        if (decision === 'deny')
            return { allow: false, reason: 'frozen by review decision' };
        return { allow: true };
    }
    /**
     ...
     */
    async reviewBlocking(ctx, agent, unit, signal) {
        const state = getOrCreateRootState(unit.rootSessionId);
     ...
     */
    async handleAsyncResult(task, result) {
        if (result.verdict === 'safe') {
            await this.audit.log({
                time: Date.now(),
    ...
    async addPendingBlock(unit, result) {
        const block = {
            id: randomUUID(),
            rootSessionId: unit.rootSessionId,
            agentId: unit.agentId,
    ...
    async ensureRootExists(rootSessionId) {
        getOrCreateRootState(rootSessionId);
    }
    async contextTokenCount(rootSessionId) {
        return withRootLock(rootSessionId, () => {
            const state = getOrCreateRootState(rootSessionId);
