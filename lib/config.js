...
function anyReviewEnabled(config) {
    return Object.values([
        ...
        config.toolResult,
        config.unmatched,
    ]).some((entry) => entry.review);
}
export function validateConfig(config) {
    if (anyReviewEnabled(config) && (config.baseURL.trim() === '' || config.model.trim() === '')) {
        throw new Error('dsh-api-review: review/block is enabled but guard API is not configured — '
